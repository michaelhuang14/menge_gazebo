 ...
 */
int MengeRand::simMain( SimulatorDBEntry * dbEntry, const std::string & behaveFile, const std::string & sceneFile, const std::string & outFile, const std::string & scbVersion, const std::string & dumpPath ) {
	size_t agentCount;
...
void MengeRand::Load(physics::WorldPtr _parent, sdf::ElementPtr _sdf) {
	 this->_world = _parent;
	simLoad();
 

  this->connections.push_back(event::Events::ConnectWorldUpdateBegin(
          std::bind(&MengeRand::OnUpdate, this, std::placeholders::_1)));
...
void MengeRand::controlActor(physics::ActorPtr _actor )
{
...
bool MengeRand::updateAgent(const Agents::BaseAgent* agt)
{
  std::stringstream sstm;
  sstm << "MengeAgent_clone_" <<agt->_id;
  std::string uniq_name=sstm.str();
...
void MengeRand::controlActorAgents()
{
        size_t numAgt=_simulator->getNumAgents();
...
void MengeRand::simUpdate(float simTime)
{
	_mengeView.pause();
	updateAgents();
	updateGoals();
...
void MengeRand::OnUpdate(const common::UpdateInfo & _info) {
	if(!agentsSynced)
	{	
